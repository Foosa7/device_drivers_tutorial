...
bool fa_BH1750::begin(byte address, TwoWire *myWire)
{
  _wire = myWire;
...
byte fa_BH1750::checkMtreg(byte mtreg)
{
...
bool fa_BH1750::start(BH1750Quality quality, byte mtreg)
{
  setQuality(quality);
...
byte fa_BH1750::calibrateTiming(byte mtregHigh, byte mtregLow)
{
...
unsigned int fa_BH1750::readChange(byte mtreg, BH1750Quality quality, bool change)
{
...
unsigned int fa_BH1750::getMtregTime(byte mtreg, BH1750Quality quality) const
{
...
unsigned int fa_BH1750::getMtregTime() const
{
...
unsigned int fa_BH1750::getMtregTime(byte mtreg) const
{
...
unsigned int fa_BH1750::getTime() const
{
  return _time;
}
...
void fa_BH1750::setQuality(BH1750Quality quality)
{
  _quality = quality;
...
bool fa_BH1750::hasValue(bool forceSensor)
{
...
void fa_BH1750::setTimeOffset(int offset)
{
  (_offset = offset);
}
...
int fa_BH1750::getTimeOffset() const
{
  return _offset;
}
...
bool fa_BH1750::writeMtreg(byte mtreg)
{
  mtreg = checkMtreg(mtreg);
  _mtreg = mtreg;
  _mtregTime = getMtregTime();
...
unsigned int fa_BH1750::getReads() const
{
  return _nReads;
}
...
unsigned int fa_BH1750::getTimeout() const
{
  return _timeout;
}

void fa_BH1750::setTimeout(int timeout)
{
  _timeout = timeout;
}
...
BH1750Timing fa_BH1750::getTiming() const
{
  return _timing;
}
...
void fa_BH1750::setTiming(BH1750Timing timing)
{
  _timing = timing;
}
...
byte fa_BH1750::convertTimeToMtreg(unsigned int time, BH1750Quality quality)
{
  float v;
  switch (quality)
  {
...
float fa_BH1750::calcLux(int raw) const
{
...
float fa_BH1750::calcLux(int raw, BH1750Quality quality, int mtreg) const
{
  float qualFak = 0.5;
...
bool fa_BH1750::saturated() const
{
...
bool fa_BH1750::adjustSettings(float percent, bool forcePreShot)
{
...
void fa_BH1750::calcSettings(unsigned int value, BH1750Quality &qual, byte &mtreg, float percent)
{
  if (percent > 100.0)
    percent = 100.0;
  _percent = percent;
...
float fa_BH1750::getPercent() const {
  return _percent;
}