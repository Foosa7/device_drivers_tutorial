//********************************************************************************************
// Description: Boilerplate code for I2C
// Copyright (c) Stefan Amrborst, 2020; Foosa Ace, 2022

#include <fa_boilerplate.h>
#include <Wire.h>

//********************************************************************************************
// Standard constructor

fa_boilerplate::fa_boilerplate()
{
}
...
bool fa_boilerplate::begin(byte address, TwoWire *myWire)
{
  _wire = myWire;
  ...
  return writeSetting(DEFAULT_SETTING); // Set standard sensitivity
}

//********************************************************************************************
// Private function. Sends command to sensor

bool fa_boilerplate::writeByte(byte b)
{
...
unsigned int fa_boilerplate::readValue()
{
  byte buff[2];
...
BH1750Quality fa_boilerplate::getQuality() const
{
  return _quality;
}
...
byte fa_boilerplate::checkMtreg(byte mtreg)
{
...
bool fa_boilerplate::start(BH1750Quality quality, byte mtreg)
{
  setQuality(quality);
...
byte fa_boilerplate::calibrateTiming(byte mtregHigh, byte mtregLow)
{
...
unsigned int fa_boilerplate::readChange(byte mtreg, BH1750Quality quality, bool change)
{
...
unsigned int fa_boilerplate::getMtregTime(byte mtreg, BH1750Quality quality) const
{
...
unsigned int fa_boilerplate::getMtregTime() const
{
...
unsigned int fa_boilerplate::getMtregTime(byte mtreg) const
{
...
byte fa_boilerplate::getMtreg() const
{
  return _mtreg;
}
...
unsigned int fa_boilerplate::getTime() const
{
  return _time;
}
...
void fa_boilerplate::setQuality(BH1750Quality quality)
{
  _quality = quality;
...
bool fa_boilerplate::hasValue(bool forceSensor)
{
  unsigned long mil = millis();
  if (!forceSensor)
  {
...
void fa_boilerplate::setTimeOffset(int offset)
{
  (_offset = offset);
}
...
int fa_boilerplate::getTimeOffset() const
{
  return _offset;
}
...
bool fa_boilerplate::writeMtreg(byte mtreg)
{
  mtreg = checkMtreg(mtreg);
  _mtreg = mtreg;
  _mtregTime = getMtregTime();
...
unsigned int fa_boilerplate::getReads() const
{
  return _nReads;
}
...
unsigned int fa_boilerplate::getRaw()
{
  if (_time > 0)
    return _value;
  yield();
...
bool fa_boilerplate::processed() const
{
  return _processed;
}
...
unsigned int fa_boilerplate::getTimeout() const
{
  return _timeout;
}

void fa_boilerplate::setTimeout(int timeout)
{
  _timeout = timeout;
}
...
BH1750Timing fa_boilerplate::getTiming() const
{
  return _timing;
}
...
void fa_boilerplate::setTiming(BH1750Timing timing)
{
  _timing = timing;
}
...
byte fa_boilerplate::convertTimeToMtreg(unsigned int time, BH1750Quality quality)
{
  float v;
  switch (quality)
  {
...
float fa_boilerplate::calcLux(int raw) const
{
...
float fa_boilerplate::calcLux(int raw, BH1750Quality quality, int mtreg) const
{
  float qualFak = 0.5;
...
bool fa_boilerplate::saturated() const
{
...
bool fa_boilerplate::adjustSettings(float percent, bool forcePreShot)
{
...
void fa_boilerplate::calcSettings(unsigned int value, BH1750Quality &qual, byte &mtreg, float percent)
{
  if (percent > 100.0)
    percent = 100.0;
  _percent = percent;
...
float fa_boilerplate::getPercent() const {
  return _percent;
}